...
class AttackAlgorithm:
    ...
    def generateValueByCross(self, currentPointer, generated_board_points, enemy_figure_indexes, val):
        ...
        while y < 8 and x < 8 :
            generated_board_points[getY(y)][getX(x)]['attack_value'].append(getVal(val, i))
    ...
    def generateValueForField(self, figure, posIdxTouple, is_white_move, generated_board_points, enemy_figure_indexes):
        ...
        if figure.lower() == 'b': # laufer
            ...
            for coord in field_coords:
                if (coord[1] < 8 and coord[1] >= 0) and (coord[0] < 8 and coord[0] >= 0) :
                    if [coord[1], coord[0]] in enemy_figure_indexes:
                        continue
                    generated_board_points[getY(coord[1])][getX(coord[0])]['attack_value'].append(generate_val('n'))
            ...
            for coord in field_coords:
                if (coord[1] < 8 and coord[1] >= 0) and (coord[0] < 8 and coord[0] >= 0) :
                    generated_board_points[getY(coord[1])][getX(coord[0])]['attack_value'].append(generate_val('k'))

            newr_val = 0.8
            # line fields
            self.generateValueByLine(currentPointer, generated_board_points, enemy_figure_indexes, [newr_val - (0.1*i) for i in range(8)])

            # cross fields 
            self.generateValueByCross(currentPointer, generated_board_points, enemy_figure_indexes, [newr_val - (0.1*i) for i in range(8)])
            ...
            for coord in field_coords:
                if (coord[1] < 8 and coord[1] >= 0) and (coord[0] < 8 and coord[0] >= 0) :
                    generated_board_points[getY(coord[1])][getX(coord[0])]['attack_value'].append(generate_val('p'))
